...
> Google Flow (ảnh/video), OmniVoice (TTS), và AI-agent CLI (Codex / Claude Code / Antigravity).
...
| Nhóm | Tùy chọn |
|------|----------|
| **AI Agent** | Agent (`codex` / `claude` / `antigravity`), model của agent (lấy từ `GET /api/agent/agents`), timeout |
...
thái phụ thuộc**: extension (Flow), OmniVoice, `codex`/`claude`/`agy`, ffmpeg — báo đỏ nếu thiếu.
...
│  │           api/ai_agent (codex/claude/agy CLI)          │  │
│  └────────────────────────────────────────────────────────┘  │
└──────┬──────────────────┬───────────────────┬───────────────┘
       │ WS :9222          │ httpx             │ subprocess/PTY
   Chrome Extension    OmniVoice (Colab)   Codex / Claude / agy CLI
       │
   Google Flow API
```
...
model trả **JSON nghiêm ngặt**, backend parse. Chạy `codex`/`claude` (stdin) hoặc `agy` (PTY).
...
(`codex`/`claude`/`antigravity`) trong Settings.
...
- Cần: extension kết nối (Flow auth), OmniVoice URL cấu hình, `codex`/`claude`/`agy` đã đăng nhập.
...
| **1. Script** ✅ | `studio/brain.py` (agent JSON + scene parser + prompt templates); `/script/generate`, `/script`, `/script/chat`, `/scenes`; ScriptTab (GeneratePanel idea+duration, Courier editor, ChatBox, scenes sidebar) | **ĐÃ XONG & verify thật:** agent sinh script từ idea (est 24s, 2 scene); chat thêm scene 3; parse + lưu scenes OK |
...
với AI-agent (codex/claude/agy) làm bộ não sinh kịch bản/mô tả/prompt, Flow sinh ảnh-video,
